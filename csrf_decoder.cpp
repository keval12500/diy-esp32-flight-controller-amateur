// ============================================================
//  Standalone CRSF parser - extracted from madflight crsf.h
//  No madflight dependencies
// ============================================================
...
class CRSFParser {
public:
    uint16_t channel[16];    // 988-2012us, directly usable
    ...
    CRSFParser() {
        buf_i = 0;
        buf_len = 0;
        for (int i = 0; i < 16; i++) channel[i] = (i == 2) ? 988 : 1500;
    }

    // Feed one byte at a time, returns true when channels updated
...
private:
    ...
    bool decode() {
        if (buf[2] != CRSF_FRAMETYPE_RC_CHANNELS) return false;
        if (buf_len != 22 + 4) return false;  // payload 22 + addr + len + type + crc
        channel_ts = millis();
    ...
    // Straight from madflight crsf.h - identical CRC table
    ...
    // Direct copy of convert_channel_value from madflight
...
// Global instance - replaces all the old PPM/CRSF code
CRSFParser crsf;
